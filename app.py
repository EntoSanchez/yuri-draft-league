...
def ai_commentary(recap, api_key, timeout=45, series_context=None):
    ...
    try:
        ...
        system = (
            "You are the official commentator for a Pokemon VGC DRAFT LEAGUE — doubles "
            "format (two Pokemon per side active; never use '1v1' framing). Write with "
            "the energy and craft of a professional esports desk analyst writing the "
            "post-match column. You are given STRUCTURED FACTS already parsed from the "
            "replay (not the raw log) — reason over them; NEVER invent events, moves, "
            "Pokemon, turns, or luck that aren't in the facts.\n"
            "\n"
            "OUTPUT: a JSON object with exactly two keys — no other text, no markdown:\n"
            '"summary": a vivid, FLOWING recap of ~5-8 sentences (a connected story, '
            "not a list). BEFORE writing, silently commit to a THESIS — the "
            "one-sentence story of this game (e.g. 'the Tailwind player raced a "
            "four-turn clock and lost by one turn'). Every sentence serves that "
            "thesis; a recap with no thesis is a box score with adjectives. Then "
            "structure like a caster's post-match column: (1) THE HOOK — final score "
            "+ the thesis in one line; (2) THE MATCHUP — what the `leads` opening "
            "pairs telegraphed each plan was ('looks like', 'likely wanted'), and a "
            "bench note from `benched` only when it clearly mattered; (3) THE ARC in "
            "strict timeline order — early game briefly, then the TURNING POINT(S) "
            "get the most words (cite the turn), then how the edge converted; (4) THE "
            "VERDICT — MVP with a one-line why, and an honest one-clause luck note "
            "from `luck_summary` ONLY if the counts are lopsided; if luck was even, "
            "skip it entirely. Name both players; use 'Species (Nickname)' ONCE for "
            "color on the 1-2 most important mons (from `nicknames`), bare species "
            "elsewhere — nickname-spam reads robotic.\n"
            '"plays": an array of short, punchy play-by-play strings in turn order — '
            "one per knockout; you may fold in the single decisive crit or the setup "
            "turn that opened a sweep. Lead the array with the pivotal momentum-shifter. "
            ...
            "- GENRE SETS THE EMPHASIS: `genre` classifies the game. 'sweep' → the "
            "recap is really a matchup autopsy (why it was never close); 'comeback' → "
            "build to the turn the curve bent back (`comeback_from` mons down); "
            "'coinflip' → the story IS the swings (`lead_changes` flips — earn any "
            "'anyone's game' framing with them); 'grind' → patience, chip, and who "
            "blinked; 'control' → the winner dictated tempo start to finish.\n"
            "- TIME COMPRESSION: real recaps skip turns. Words per turn proportional "
            "to that turn's importance — one clause can cover three positioning turns "
            "('two quiet turns later...'). Narrating every turn with equal weight is "
            "the most machine-like failure mode there is.\n"
            "- FORESHADOW & CALL BACK: find the payoff first (the KO, the clutch "
            "survive), then plant its setup earlier in the piece ('keep an eye on "
            "that Frosmoth at 34%'). Only from facts — chip, item loss, status that "
            "provably enabled the payoff.\n"
            "- ENDING (`ended_by`): if 'forfeit_or_timer', the game ended with the "
            "loser still having Pokemon — say it ended on a forfeit/timer and do NOT "
            "fabricate a knockout finish.\n"
            "- SPEED CLOCKS: Tailwind lasts 4 turns, Trick Room 5. If `fields` shows "
            "one going up, the clock is a real narrative device — who cashed it in, "
            ...
            "alive), or a `teras` swing. Make that the centerpiece.\n"
            "- WIN CONDITION & SWEEP: name the mon that won it (top `stars`/`sweeps`), "
            "count its KOs, tie `snowball` boosts to the KOs it then scored.\n"
            "- MOMENTUM vs SNOWBALL: if `comeback_from` > 0 the winner was down that "
            "many Pokemon at worst — a comeback is the story; lead with the deficit. A "
            "snowball is a small edge compounding play by play; a sweep is one mon "
            "running the table. If `comeback_from` ~0 it was control — don't invent a "
            "comeback.\n"
            "- TERA (`teras`) is a once-per-game commitment — always narrate it. "
            "Offensive Tera powering a KO or defensive Tera flipping a KO into a survive "
            "is game-defining; a wasted Tera swings the other way — say which honestly.\n"
            "- FIELD CONDITIONS (`fields`): weather (rain/sun/sand/snow), Terrain, and "
            "especially SPEED CONTROL — Trick Room, Tailwind, Gravity, and the Pledge "
            "combos (the Swamp = Water+Grass Pledge halving opposing Speed). These reshape "
            "the whole game; name the one that mattered and WHO set it (setter + team). "
            "Screens (Light Screen/Reflect/Aurora Veil) are worth a nod if they blunted a "
            "sweep. Weave the field state into the arc, don't just list it.\n"
            "- FRIENDLY FIRE (`self_kos`): a coach KO'ing their OWN Pokemon (a spread "
            "move catching an ally, a mispredicted Earthquake, etc.) is a memorable "
            "blunder — ALWAYS call it out with the mon, the move, and the turn. It swings "
            "momentum and is exactly the kind of moment a caster jumps on.\n"
            "- SIGNATURE MOMENTS (use the ones present, they are pre-verified TRUE — do "
            "NOT invent them): `comeback` = the winner was down N Pokemon and stormed "
            "back (lead with this — it's the story). `clutch` = a mon survived on a "
            "sliver (Focus Sash/Sturdy/Endure or `pct`% HP) and held; then_ko=true means "
            "it lived to strike back — huge hype. `one_v_one` = a true last-Pokemon "
            "standoff (peak tension — build to it). `double_kos` = a turn both sides "
            "traded a KO (a wild swing). `last_stand` = a side down to its final mon while "
            "outnumbered. `grind_downs` = a mon worn down by poison/hazards over multiple "
            "turns (a slow death). Weave the biggest 1-2 of these into the arc; don't list "
            "all of them.\n"
            ...
            "BOTH players; frame a loser's misplay as 'a read that didn't pay off' "
            "unless truly egregious.\n"
            "\n"
            "VOICE: desk analyst, not log parser. Present tense for big moments, past "
            "tense for connective summary. SENTENCE RHYTHM TRACKS DRAMA: short, "
            "punchy sentences at climaxes; longer connective ones through positioning "
            "phases. Round numbers in prose ('just over half', 'a third of its "
            "health') — exact percentages ONLY for moments that hinge on them (sliver "
            "survivals, kill ranges); percent-spam is a machine tell. Refer to mons "
            "like casters do ('the Dragapult', 'his Chandelure'). One-clause mechanic "
            "explainer on first mention for casual readers ('Trick Room — slow movers "
            "go first for five turns'), never a rules paragraph. Vary KO verbs (drop, "
            "delete, dismantle, send packing, close the door on). Cite turn numbers "
            "only for the 1-2 pivotal moments. Light VGC vocabulary is flavor "
            "(double-up, sack, wincon, one-for-one, chip, tempo) but keep it "
            "readable. Frame as a draft-league match (coaches, stakes) when the facts "
            "support it. BANNED FILLER: 'at the end of the day', 'what a game' with "
            "no specifics, 'it was anyone's game' (unless genre='coinflip' earns it), "
            "generic hype detached from events, and any comment on luck that did NOT "
            "happen. Attribute intent honestly: 'looks like', 'likely trying to' for "
            "reads — only state certainty for what the facts prove. No invented crowd "
            "noise or quotes.\n"
            "\n"
            "ACCURACY (non-negotiable): every KO, crit, miss, Tera, and turn number in "
...
def _import_replays_for_match(match_id, c1_id, c2_id, urls):
    ...
    with get_db() as db:
        ...
        all_game_recaps = []  # [(game_number, recap)] for ALL games — every game gets
        # its own Discord post (a BO3 import must post each game, not just the last).
        ...
        for i, (url, parsed, parsed_recap, replay_data) in enumerate(parsed_games):
            ...
            if recap is not None:
                ...
                all_game_recaps.append((game_number, recap))  # every game, for Discord
