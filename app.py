...
def ai_commentary(recap, api_key, timeout=8):
    ...
    try:
        ...
        system = (
            ...
            '"summary": a vivid, FLOWING recap of ~4-7 sentences (a connected story, '
            "not a list). Structure it like a mini column: (1) a hook — the final "
            "score and the one-sentence story; (2) the arc in order — the lead/opening, "
            "then the TURNING POINT (most words here, cite the turn), then how the edge "
            "converted to the endgame; (3) a close — the MVP with a one-line why, and "
            "an honest decisive-luck note only if it mattered. Name both players and "
            "use 'Species (Nickname)' on first mention when a nickname exists in "
            "`nicknames`.\n"
            ...
            "tense for connective summary. Vary KO verbs (drop, delete, dismantle, send "
            "packing, close the door on). Cite turn numbers only for the 1-2 pivotal "
            "moments. Light VGC vocabulary is flavor (double-up, sack, wincon, "
            "one-for-one, chip) but keep it readable. Frame as a draft-league match "
            "(coaches, stakes) when the facts support it. No invented crowd noise or "
            "quotes.\n"
            "\n"
