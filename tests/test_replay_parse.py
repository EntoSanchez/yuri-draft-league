...
def test_recap_shows_mega_ko_full_bring():
    ...
    nmap = {
        n: R.resolve_poke_name(n, roster)
...
# ── Charge / two-turn move KO attribution ──────────────────────────────────────
# A two-turn charge move (Electro Shot, Solar Beam, Fly, Dig, …) logs its RELEASE
# with an empty target on the |move| line; the real hit target only appears on the
# following |-anim| line. The KO must credit the charge-move user — but ONLY when a
# real |-damage| lands, so a Protect-blocked charge never mis-credits a later faint.


def test_charge_move_ko_credits_user():
    """Electro Shot (empty |move| target, real hit via |-anim|) credits its user."""
    log = "\n".join(
        [
            "|player|p1|Alice",
            "|player|p2|Bob",
            ...
            "|move|p1a: Ar|Electro Shot||[still]",  # charge, no target
            "|-prepare|p1a: Ar|Electro Shot",
            "|-anim|p1a: Ar|Electro Shot|p2a: Rilla",  # real target here
            "|-damage|p2a: Rilla|0 fnt",  # real damage lands
            "|faint|p2a: Rilla",
            "|win|Alice",
        ]
    )
    p = R.parse_log(log)
    assert p["kills"]["p1"] == {"Archaludon": 1}
    assert p["deaths"]["p2"] == {"Rillaboom": 1}


def test_charge_blocked_then_perish_no_false_credit():
    """A charge blocked by Protect (no -damage) then a Perish Song self-KO must NOT
    credit the charge-move user (adversarial-review finding)."""
    log = "\n".join(
        [
            "|player|p1|Alice",
            "|player|p2|Bob",
            "|switch|p1a: Ar|Archaludon, M|100/100",
            "|switch|p2a: Poli|Politoed, M|100/100",
            "|move|p1a: Ar|Electro Shot||[still]",
            "|-prepare|p1a: Ar|Electro Shot",
            "|-anim|p1a: Ar|Electro Shot|p2a: Poli",
            "|-activate|p2a: Poli|move: Protect",  # blocked, NO -damage
            "|-start|p2a: Poli|perish0",
            "|faint|p2a: Poli",  # Perish Song self-KO
            "|win|Alice",
        ]
    )
    p = R.parse_log(log)
    assert p["kills"]["p1"] == {}, f"false credit: {p['kills']['p1']}"


def test_charge_blocked_then_other_mon_kills():
    """Blocked charge stale-pending, then a DIFFERENT mon lands the real KO."""
    log = "\n".join(
        [
            "|player|p1|Alice",
            "|player|p2|Bob",
            "|switch|p1a: Ar|Archaludon, M|100/100",
            "|switch|p1b: Sw|Swampert, M|100/100",
            "|switch|p2a: Poli|Politoed, M|100/100",
            "|move|p1a: Ar|Electro Shot||[still]",
            "|-anim|p1a: Ar|Electro Shot|p2a: Poli",
            "|-activate|p2a: Poli|move: Protect",  # Archaludon's charge blocked
            "|move|p1b: Sw|Earthquake|p2a: Poli",  # Swampert lands the real KO
            "|-damage|p2a: Poli|0 fnt",
            "|faint|p2a: Poli",
            "|win|Alice",
        ]
    )
    p = R.parse_log(log)
    assert p["kills"]["p1"] == {"Swampert": 1}, f"wrong killer: {p['kills']['p1']}"


def test_real_fixture_58_winner_has_four_kos():
    import os
